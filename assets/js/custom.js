...
function loading(){
    ...
    function updateProgress(){
        ...
        //     {ease: Back.easeOut.config(1.7), delay: 2, scale:0}, // staggerFromto를 쓰면 css에 미리 설정안해놓아도 초기값 설정 가능
        ...
        if(imgCurrent > 99){ // 99가 되면 안올라가기 때문에 100으로 바꿔줌
            imgCurrent = 100;
        }
    }
}
...
function contents(){    
    if(mode === 'v'){
        setTimeout(() => {
            ...
            // resize시 각 섹션 요소별 offset값 초기화 하는 함수
            ...
            const currMenuCheck = () => {
                const sct = $(window).scrollTop();
                if(gWidthSize < 768){
                    if(sct + gHeightSize/2 > $('.sec8').offset().top ){
            ...
            $('.menu ul li a').click(function(e){
                ...
                if(gWidthSize > 768){
                    ...
                    if(!(index === 0)){
                        if(index === 6){
                            let secOffset = $('.section').eq(7).offset().top ;
                            $('body, html').animate({
                                scrollTop: secOffset
                            })
                        }else{
        ...
        // 레이아웃을 가로로 짜고 그 레이아웃의 총 가로길이를 body의 height값으로 해주는것.
        let lastScrollTop = 0;                       // 현재 스크롤값( 제일 마지막 스크롤 값 )
        let roadLength = $('#hContents').width();     // 전체 가로값
        $('body').css('height',roadLength);          // 섹션 전체 가로값을 body에 height 값으로 할당
        ...
        $(window).scroll(() => {
            ...
            let dHeight = $('body').height() - gHeightSize; // 전체 body의 height값 에서 현재 height값 빼줌
